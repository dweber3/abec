...
myPeptides <- read.table(peptidesFile,header=TRUE)
...
myDup2 <- read.table(state1DFracFile,header=TRUE)
...
myD_SD1 <- read.table(state0SDFile,header=TRUE)
myD_SD2 <- read.table(state1SDFile,header=TRUE)
...
myOutput$pvaluedm <- 2*pt(abs(nobs*myOutput$DifMeans/(2*myOutput$Sdm)),2*nobs-2,lower.tail = FALSE, log.p = FALSE)
...
myOutput$pvaluebc <- 2*pt(abs(nobs*myOutput$Areabc/(2*myOutput$Sbc)),2*nobs-2,lower.tail = FALSE, log.p = FALSE)
myOutput
}
